...
def bag_generator(image,num_instance,total_size =256):
    num_instance_row = int(total_size/(num_instance**(1/2)))
    bag = []
    for instance_idx in range(num_instance):
        bag.append([])
        start_idx_row = (num_instance_row * instance_idx )% total_size
        start_idx_column = math.ceil((num_instance_row * instance_idx) /total_size )
        
        for i in range(image.shape[0]):
            bag[instance_idx].append([])
            for row_id in range(num_instance_row):
                bag[instance_idx][i].append([])
                for column_id in range(num_instance_row):
                    bag[instance_idx][i][row_id].append(image[i][start_idx_row+row_id][start_idx_column+column_id])
                    
    bag = np.array(bag)
    return bag

def featurize_bag(bag,modules):
    
    
    num_instance = bag.shape[0]
    
    featurized_bag = []
    
    for i in range(num_instance):
        out = modules[0]( torch.from_numpy(bag[i].reshape(1,bag[i].shape[0],bag[i].shape[1],bag[i].shape[2])))
        out = modules[1][0](out)
        out = modules[1][1](out)
        out = modules[1][2](out)
        features = modules[2](out)
       
        ##featurized_bag.append(features.reshape(-1).tolist())
        featurized_bag.append(features.tolist())
        
#     featurized_bag = np.array(featurized_bag)
    
    return featurized_bag
...
train_options['train_list'] = [
    "20180108T184332_dmi_prep.nc",
    "20180122T205424_dmi_prep.nc",
"20200213T203546_dmi_prep.nc",
"20200224T102034_cis_prep.nc",
"20200412T101835_cis_prep.nc",
"20200412T110426_dmi_prep.nc"]
...
class mil_nn(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.up4(x)
        x_1 = self.up5_1(x)
        x_1 = x_1.view(-1,256,256)
        x_2 = self.up5_2(x)
        x_2 = x_2.view(-1,256,256)
        x_3 = self.up5_3(x)
        x_3 = x_3.view(-1,256,256)
        
        return {"SIC":x_1,"SOD":x_2,"FLOE":x_3}
...
# Setup U-Net model, adam optimizer, loss function and dataloader.
mil_nn = mil_nn()
optimizer = torch.optim.Adam(list(mil_nn.parameters()), lr=train_options['lr'])
...
# loss_functions = {chart: torch.nn.CrossEntropyLoss(ignore_index=train_options_MIL['class_fill_values'][chart]) \
#                                                    for chart in train_options_MIL['charts']}
loss_functions = {chart: torch.nn.CrossEntropyLoss() \
...
for epoch in tqdm(iterable=range(train_options['epochs']), position=0):
    ...
    mil_nn.train()  # Set network to evaluation mode.
    ...
    for i, (batch_x, batch_y) in enumerate(tqdm(iterable=dataloader, total=train_options['epoch_len'], colour='red', position=0)):
        ...
        # Convert data into bag
        batch_length = batch_x.shape[0]
        batch_bags = []
        for batch_id in range(batch_length):
            bag = bag_generator(batch_x[batch_id],num_instance=num_instance,total_size =256)
            featured_bag = featurize_bag(bag,modules)
            
            
            featured_bag = np.array(featured_bag)
            featured_bag = torch.from_numpy(featured_bag)
            featured_bag = featured_bag.float()
            m = nn.MaxPool2d(1, stride=1)
            
            featured_bag = torch.reshape(featured_bag,(featured_bag.shape[0]*featured_bag.shape[1]*featured_bag.shape[2],featured_bag.shape[3],featured_bag.shape[4]))
            aggregated_bag = m(featured_bag)
            batch_bags.append(aggregated_bag.tolist())
        ...
        with torch.cuda.amp.autocast():
            # - Forward pass. 
            output = mil_nn(batch_x)

            # - Calculate loss.
            for chart in train_options['charts']:
                loss_batch += loss_functions[chart](input=output[chart], target=batch_y[chart].to(device).float())
    ...
    mil_nn.eval()  # Set network to evaluation mode.
    ...
    file_name  = "best_model_mil_" + str(num_instance)
    if combined_score > best_combined_score:
        best_combined_score = combined_score
        torch.save(obj={'model_state_dict': mil_nn.state_dict(),
...
# Setup U-Net model, adam optimizer, loss function and dataloader.
net = UNet(options=train_options).to(device)
net.load_state_dict(torch.load(file_name)['model_state_dict'])
print('Model successfully loaded.')
